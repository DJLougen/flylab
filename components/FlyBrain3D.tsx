...
export function FlyBrain3D({ laterality, driveActive, perturbation, conditionLabel, timeMs }: FlyBrain3DProps) {
  ...
  const activity = useMemo(() => circuitActivityFor(laterality, driveActive), [laterality, driveActive]);
  ...
  useEffect(() => {
    ...
    const labels = [
      ...
      makeTextSprite('VNC / T3 · SCHEMATIC SHELL', '#819397', [0, -6.55, 0]),
    ...
    sceneApiRef.current = {
      ...
      applyVisualState: (state) => {
        ...
        for (const [id, render] of neuronRenders) {
          ...
          const color = activeMdn ? 0xd49cff : structuralTarget ? 0x6ee7d2 : selected ? 0xf2ffff : cell?.cell_type === 'MDN' ? 0x6a9696 : 0x6d86a6;
          render.lineMaterial.color.set(color);
          render.lineMaterial.opacity = activeMdn ? 0.98 : structuralTarget ? 0.88 : selected ? 0.82 : 0.22;
          render.glowMaterial.color.set(color);
          render.glowMaterial.size = activeMdn ? 0.055 : structuralTarget ? 0.047 : 0.04;
          const glowOpacity = activeMdn ? 0.48 : structuralTarget ? 0.3 : selected ? 0.2 : 0;
        ...
        for (const [id, shell] of shellRenders) {
          ...
          shell.material.color.set(isBrainTarget ? 0xd49cff : isT3Path ? 0x6ee7d2 : shell.baseColor);
          shell.material.opacity = isBrainTarget ? 0.15 : isT3Path ? 0.17 : id.includes('Optic') ? 0.045 : 0.065;
        }
      },
    };
    ...
    const findHit = (event: PointerEvent) => {
      ...
      return raycaster.intersectObjects([...neuronRenders.values()].map((record) => record.line), false)[0]?.object.userData.neuronId as string | undefined;
    };
  ...
  }, [activity.activeMdnIds, activity.highlightedLbl40Ids, hoveredId, loadState, selectedId, shellVisible]);

  return (
    <section className="fly-brain-viewer" aria-label="Interactive BANC v888 MDN and LBL40 circuit viewer">
