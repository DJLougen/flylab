...
The current suite contains 45 tests. It supplies a compatible `modelContext`, verifies that exactly eight tools register, checks their closed schemas and current annotations, invokes registered tools through the shared handler, rejects unexpected inspector fields, and confirms that aborting the registration signal disposes all tools. Pure agent-context tests cover every artifact transition, the person-only gate, simulation and evidence-save waiting states, exact comparison-lineage recovery, canonical stage derivation, completion, fixed null/array fields, and the one-next-action rule. The suite also covers mandatory controls, onset-referenced/null response latency, canonical input ordering, exact protocol identity and bounds, silence labeling, stable proposal/bundle identity, and the complete analysis panel.
...
For an end-to-end verification of all eight tools and the approval boundary, run `FLYLAB_VERIFY_WORKFLOW=1 npm run verify:webmcp`. This invokes all eight registered contracts through Chrome's real WebMCP protocol; it does not use the guided-example button. The isolated test verifies the inspector before discovery, at the blocked person-only gate, after approval, after workflow completion, and again after a person edits the exact protocol. It confirms that simulation is blocked with `APPROVAL_REQUIRED` and clicks the approval control colocated with the complete visible protocol through the DOM. Before the successful run, it exercises two post-start simulation-cancellation paths:
