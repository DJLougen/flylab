...
- laterality: bilateral for GF; bilateral, left, or right for MDN
- controlled design: baseline, model-sham, and primary perturbation, plus unilateral comparisons only where the selected motor map supports them
...
The competition evidence chain is GF-first:

1. Measured literature records describe assay-scoped adult GF activation, silencing, recording, and short-mode escape effects.
...
The secondary MDN slice retains measured perturbation/laterality records and a pinned BANC structural inventory. Its four directed MDN→LBL40 rows total 153 v3-predicted synaptic links after the postsynapse-size ≥10-voxel filter; this future-work v3 product is distinct from the v2 (≥5) product used in the paper analyses.
...
The operational lineage is equally strict. Discovery returns the full source closure required by the selected circuit and motor map, while `matches_requested_evidence_labels` and the eligible-ID lists preserve the active filter. It also persists the complete discovery decision and candidate records. A hypothesis may cite only the selected circuit and filtered discovered IDs. Changing the filter invalidates a hypothesis that no longer belongs to that selection. A trial's target and perturbation must match the saved hypothesis. A comparison accepts analyses from exactly one batch and requires its objective metric in each analysis. Saving requires the current hypothesis, experiment, sole batch, comparison, and exactly the comparison's complete analysis-ID set. Experiment scope serializes that selected lineage; mission scope additionally serializes the untrusted goal, discovery decision, considered/rejected alternatives, exclusions, and coverage gaps. The local model card is related as `method_definition`; the FlyGym paper and pinned release are related only as `embodiment_reference` and do not define FlyLab's equations.
...
- base random seed
- common-random-number seed-policy version and formulas
- analysis metric-method version
- model, controller, environment, and boundary strings
- model assumptions

For zero-based replicate index `r`, every condition reuses the same latent draw seed:
...
Every successful tool call returns its `page_session_id`, previous revision, and current `state_revision`. Every state-changing call must echo the inspected page session and expected revision. A wrong session or revision publishes no requested mutation and returns non-retryable `STALE_STATE` with `inspect_flylab_state` as recovery. `run_fly_simulation` and `save_fly_evidence` also capture the revision before preparation and compare it with live state immediately before commit. Failure/cancellation activity never rewinds the newer mission's stage.
