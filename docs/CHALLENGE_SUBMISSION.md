...
FlyLab was created on August 26, 2026 during the challenge period. The dated [first public commit](https://github.com/DJLougen/flylab/commit/a45eb82ad29d62a1bf7afc0aff89f71a70384db9) records the initial challenge build. Its primary audience is a computational-neuroethology researcher or educator reviewing source-backed virtual brain-to-body experiments. The concrete failure it prevents is an agent silently continuing from stale page state or presenting a simulation result after its evidence lineage, protocol, or human approval has changed. Agent-tool builders are a secondary audience.
...
FlyLab gives agents one read-only state inspector and seven WebMCP scientific actions that turn adult fly brain-to-body questions into cited hypotheses, human-approved virtual experiments, reproducible analyses, bounded follow-ups, and provenance-rich evidence bundles inside one shared page session.

**Submission description**

FlyLab is an agent-native virtual neuroethology lab built around two scientifically transparent adult *Drosophila* brain-to-body vertical slices: MDN into six-leg reverse walking and Giant Fiber/DNp01 into a bilateral short-mode escape controller with middle-leg jump plus wing-depressor output. Instead of asking an agent to guess its way through a complex brain viewer, the site exposes one read-only control-plane inspector plus seven structured scientific actions: find curated circuits, draft a falsifiable hypothesis, design controls, run an approved simulation, quantify behavior, compare trials, and save the evidence lineage.
...
FlyLab's current embodiment is the deterministic reduced-order mapped-motor model version `0.2.0`, using `mapped-circuit-to-body-adapter.v1`. Its numeric parameters and circuit-to-controller bindings are hand-authored and uncalibrated; the public [model card](MODEL_CARD.md) gives every equation and constant. It is not FlyGym, a connectome execution, a complete fly brain or body, or a wet-lab experiment. Sources, assumptions, dataset and model versions, typed motor paths, seeds, run IDs, analyses, limitations, and a manifest hash are saved together so a compelling result never loses its boundary. The WebMCP surface follows OpenAI's [Site tools documentation](https://learn.chatgpt.com/docs/webmcp).

The circuit view is an orbitable Three.js rendering. For MDN, it shows six pinned, reconstruction-derived BANC v888 L2 skeletons—four MDNs and two LBL40 cells—with camera presets and accessible cell inspection. Cyan marks four directed MDN→LBL40 structural rows totaling 153 v3-predicted synaptic links after the released postsynapse-size ≥10-voxel filter. For GF, it shows a separately labeled literature schematic of the GF→TTMn/TTM jump-leg and GF→PSI→DLMn/DLM wing branches; no reconstruction or dataset neuron ID is invented. Purple replay illumination always marks selected model targets, not measured activity or biological signal propagation.

The open-field arena renders the adult fly itself in Three.js. Its major external landmarks are research-informed, while its mesh and appendage movements remain explicitly schematic. The selected motor map routes MDN output into a six-leg walking cue or GF output into T2-leg extension, wing tuck/downstroke, and model lift. The replayed position, heading, vertical displacement, and target window come from the reduced-order simulation; the body is not a scan, a FlyGym execution, a muscle model, or a biomechanical reconstruction.
...
| **Execution** | The no-login HTTPS application delivers coherent adult MDN and GF brain-to-body slices with an interactive Three.js arena and circuit viewer, controlled seeded simulation, analysis, bounded follow-up, and evidence export. The release is covered by public CI and a complete Chrome WebMCP workflow against the deployed site. |
...
- Curated adult MDN and Giant Fiber/DNp01 evidence records with primary-source links
- Schema- and runtime-mandatory baseline/model-sham controls; bilateral MDN designs add left-only and right-only comparisons, while the current GF map is bilateral-only
...
- A complete five-metric panel declared by each motor map, plus an always-reported motor-program response summary; no-response latency is JSON `null` / UI `n/a`
...
- [von Reyn et al., *Nature Neuroscience* (2014)](https://doi.org/10.1038/nn.3741) — targeted GF activation, silencing, recording, and short-mode escape; publisher copyright.
- [King & Wyman, *Journal of Neurocytology* (1980)](https://doi.org/10.1007/BF01205017) — primary anatomy of the GF jump-muscle and flight-muscle branches; publisher copyright.
- [Allen & Murphey, *European Journal of Neuroscience* (2007)](https://doi.org/10.1111/j.1460-9568.2007.05686.x) — mixed GF–TTMn synapse modality; CC BY-NC 2.5.
- [Azevedo et al., *Nature* (2024)](https://doi.org/10.1038/s41586-024-07389-x) — adult-female FANC structural context for coordinated leg/wing escape; no FANC rows are bundled.
...
- [x] Re-clone the public GitHub repository and pass dependency installation, all 74 tests, lint, and production build from only the published files.
