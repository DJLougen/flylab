...
FlyLab gives agents eight native WebMCP tools that turn an adult fruit-fly rapid-escape question into a cited hypothesis, an immutable operator-authorized virtual protocol, per-run auditable analysis, a bounded follow-up, and a scoped v3 evidence bundle inside one shared page session.
...
Each simulation run exposes its exact seed, outputs, trajectory ID, trajectory seed, event timeline, and full state trajectory. The Three.js arena renders the exact selected seeded run. The older condition-level `illustrative_condition_replay` is a compatibility artifact only and is excluded from analysis and the primary visual audit. `flylab.behavior-metrics.v5` derives every condition summary from the authoritative per-run trajectories and publishes each metric's formula, unit, sign convention, aggregation, null rule, window semantics, provenance, and interpretation boundary plus per-run audit rows. The explicit state branches are MDN `stance → preparation → reverse_walk → recovery` and GF `stance → preparation → jump → wing_deployment → airborne → recovery`; a threshold crossing too late to express its body sequence is reported as censored and produces no body output.

Evidence can be saved as `flylab.experiment-evidence-bundle.v3` for one exact lineage or `flylab.mission-evidence-bundle.v3` for that lineage plus the untrusted goal, discovery decision, considered and rejected alternatives, exclusions, and coverage gaps. The exact payload is returned in a schema-version-3 evidence-export envelope documented by the deployed [portable export JSON Schema](https://flylab-neuroethology.d-lougen.chatgpt.site/schemas/flylab-evidence-export-v3.schema.json). Its checksum detects changes; it is not a signature or immutability guarantee.

FlyLab's mapped-motor model `0.3.0` uses `state-coherent-mapped-circuit-adapter.v2`, `stateful-open-field-model-scale.v3`, and `flylab.behavior-metrics.v5`. GF state-transition order and approximate event intervals are constrained by cited adult escape measurements, but response probabilities, body amplitudes, controller gains, recovery timing, and all MDN dynamics remain hand-authored and unfitted. It does not execute FlyGym, BANC/FANC/MANC, synapses, neural dynamics, muscles, aerodynamics, or a wet-lab experiment. The GF circuit view is literature-schematic and has no invented dataset neuron IDs. The separate MDN/LBL40 view contains six reconstruction-derived BANC v888 L2 skeletons. Purple marks model selection, never measured activity.

The legacy FNV-1a `runHash` commits only the run/trajectory identity list. A separate SHA-256 `runContentHash` commits the approved protocol, model manifest, and complete condition-run content, and the analysis binds that content hash.
...
After the operator approves the exact visible protocol, use:
...
- State-coherent body-event trajectories with explicit threshold-crossing, censoring, contact, and recovery state
- Exact selected seeded-run replay; legacy illustrative condition replays retained only for compatibility and excluded from analysis
- Legacy identity-only FNV `runHash` plus SHA-256 `runContentHash` over protocol, model, and complete runs
- Formal five-metric panels plus separate response-initiation and threshold/censoring summaries
...
- Muscle mechanics, aerodynamics, fitted probabilities/amplitudes/controller gains/recovery, calibrated biological effect sizes, or fitted MDN dynamics
